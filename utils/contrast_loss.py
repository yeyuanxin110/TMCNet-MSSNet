...
class NTXentLoss(nn.Module):
    ...
    def forward(self, zi, zj, m, n):
        
        zi, zj = F.normalize(zi, dim=1), F.normalize(zj, dim=1)
        m, n = F.normalize(m, dim=1), F.normalize(n, dim=1)
        bs = zi.shape[0]
        for i in range(bs):
            if i==0:
                #zi0 = torch.cat([zi, 0.1 * m], dim=1)
                #zj0 = torch.cat([zj, 0.1 * n], dim=1)
                zi0=zi+m
                zj0=zj+n
                z_all = torch.cat([zi0, zj0], dim=0)  # input1,input2: z_i,z_j
                # [2*bs, 2*bs] -  pairwise similarity
                if self.use_cos_sim:
                    sim_mat = torch.exp(self.cosine_similarity(
                        z_all.unsqueeze(1), z_all.unsqueeze(0)) / self.tau)  # s_(i,j)
                ...
                m = torch.from_numpy(m).to(0)
                #zi1 = torch.cat([zi, 0.1*m], dim=1)
                zi1=zi+0.1*m
                n = n.detach().cpu().numpy()
                n = np.roll(n, 1, axis=0)
                n = torch.from_numpy(n).to(0)
                #zj1 = torch.cat([zj, 0.1*n], dim=1)
                zj1=zj+0.1*n
                '''
                z_all = torch.cat([zi0, zj1], dim=0)  # input1,input2: z_i,z_j
                # [2*bs, 2*bs] -  pairwise similarity
                if self.use_cos_sim:
                    sim_mat = torch.exp(self.cosine_similarity(
                        z_all.unsqueeze(1), z_all.unsqueeze(0)) / self.tau)  # s_(i,j)
                ...
                # loss = -(torch.mean(torch.log(pos / (pos + neg))))
                # [2*bs, 2*bs-1]
                sim_neg1 = sim_mat.masked_select(self.neg_mask).view(2 * bs, -1)
                #sim_neg=torch.cat([sim_neg,sim_neg1],1)
                sim_neg = sim_neg+sim_neg1'''


                z_all = torch.cat([zi1, zj0], dim=0)  # input1,input2: z_i,z_j
                # [2*bs, 2*bs] -  pairwise similarity
                if self.use_cos_sim:
                    sim_mat = torch.exp(self.cosine_similarity(
                        z_all.unsqueeze(1), z_all.unsqueeze(0)) / self.tau)  # s_(i,j)
                ...
                sim_neg2 = sim_mat.masked_select(self.neg_mask).view(2 * bs, -1)
                #sim_neg = torch.cat([sim_neg, sim_neg2], 1)
                sim_neg = sim_neg + sim_neg2
