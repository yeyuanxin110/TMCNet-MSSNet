...
def filp_array(array, flipCode):
    '''Filp an [HW] or [HWC] array vertically or horizontal according to flipCode.根据flipCode垂直或水平填充一个[硬件]或[HWC]数组'''
    if flipCode != -1:
...
def random_resized_crop(image,image_sar,label,size=(224,224), scale=(0.25, 0.75), ratio=(3./4., 4./3.)):
    ...
    if len(image.shape) > 2:
        ...
        (height, width), channel = image.shape, 1
    out_img=np.zeros((size[0], size[1], channel), dtype=np.uint8)
    out_img2 = np.zeros((size[0], size[1], channel), dtype=np.uint8)
    ...
    for attempt in range(5):
        ...
        if w < W and h < H:
            ...
            image_sar=image_sar[i:i+h, j:j+w]
    ...
    if channel>3:
        ...
        for i in range(channel):
            ...
            out_img2[:, :, i] = cv2.resize(image_sar[:, :, i], size)

    else:
        out_img=cv2.resize(image,size)
        out_img2 = cv2.resize(image_sar, size)
        label=cv2.resize(label,size, interpolation=cv2.INTER_NEAREST)
    return out_img,out_img2,label
...
def randomHueSaturationValue(image,image_sar,
    ...
    if np.random.random() < u:
        ...
        image_sar = cv2.cvtColor(image_sar, cv2.COLOR_RGB2HSV)
        h1, s1, v1 = cv2.split(image_sar)
        ...
        image_sar = cv2.merge((h1, s1, v1))
        #image = cv2.merge((s, v))
        image = cv2.cvtColor(image, cv2.COLOR_HSV2RGB)
        image_sar = cv2.cvtColor(image_sar, cv2.COLOR_HSV2RGB)

    return image,image_sar
...
def join_transform(image,image_sar,label1,input_size=(224,224)):
    """
    ...
    数据增强功能，用于使用多个波段训练 RS 影像。
    参数：
        图像：具有多个频段的 RS 图像的 [HWC] ndarray
        标签： [HW1] 标签数组
    """
    if image.shape[1]>256:
        image = random_crop_pair(image,  [256,256])
    if image_sar.shape[1]>256:
        image_sar = random_crop_pair(image_sar,  [256,256])
    image1=image.copy()
    #label1=np.array(range(256*256)).reshape(256,256)
    #label1 = np.expand_dims(label1, axis=2)
    label2 = label1.copy()
    image_sar1=image_sar.copy()
    image,image_sar,label1 = random_resized_crop(image,image_sar,label1,size=input_size,scale=(0.4,1.0))
    image1,image_sar1,label2 = random_resized_crop(image1,image_sar1,label2,size=input_size,scale=(0.4,1.0))
    band_num = image.shape[2]
    # Random Flip
    f = [1, 0, -1, 2, 2][np.random.randint(0, 5)]  # [1, 0, -1, 2, 2]
    if f != 2:
        image1= filp_array(image1, f)
        image_sar1 = filp_array(image_sar1, f)
    ...
    if np.random.random() < 0.8:
        ...
        image_sar1 = np.rot90(image_sar1, k, (1, 0))
        label2 = np.rot90(label2, k, (1, 0))
    
    #k = np.random.randint(0, 4)  # [0, 1, 2, 3]
    #image1 = np.rot90(image1, k, (1, 0))  # clockwise
    if np.random.random() < 0.8:
        image1,image_sar1=color_aug(image1,image_sar1)
    #if np.random.random() < 0.8:
        #image=color_aug(image)
    #image=random_image_to_gray(image,p=0.1)
    image1,image_sar1=random_image_to_gray(image1,image_sar1,p=0.1)#随机灰度化（仅保留其中一个通道）
    if np.random.random() < 0.5:
        image1,image_sar1=random_GaussianBlur(image1,image_sar1)



   
    return image,image1,image_sar,image_sar1,label1,label2

def join_transform_v1(image,image_sar,  input_size=(224,224)):
    """
    Data augment function for training RS imagery with mutiple bands.
    Args:
        image: [HWC] ndarray of RS image with mutiple bands
   ...
   # print(image.shape)
    image,image_sar,label1 = random_resized_crop(image,image_sar,size=input_size,scale=(0.4,1.0))
    image1,image_sar1,label2 = random_resized_crop(image1,image_sar1,label2,size=input_size,scale=(0.4,1.0))
    band_num = image.shape[2]
    # Random Flip
    f = [1, 0, -1, 2, 2][np.random.randint(0, 5)]  # [1, 0, -1, 2, 2]
    ...
    if np.random.random() < 0.25:
        ...
        for c in range(0, band_num):
            ...
            image_sar1[:, :, c] = randomColorAugment(image_sar1[:, :, c], 0.2, 0.2)
    if np.random.random() < 0.25:
        band_num=image1.shape[2]
        for c in range(0, band_num):
            if band_num >= 3:
                ...
                image_sar1[:, :, :3] = randomHueSaturationValue(image_sar1[:, :, :3], hue_shift_limit=(-30, 30),
                                                            sat_shift_limit=(-5, 5),
                                                            val_shift_limit=(-15, 15))
    if np.random.random() < 0.25:
        band_num=image1.shape[2]
        for c in range(0, band_num):
            ...
            image_sar1[:, :, c] = randpixel_noise(image_sar1[:, :, c], p=p)
        
    
    image1=random_image_to_gray(image1,p=0.1)
    image_sar1 = random_image_to_gray(image_sar1, p=0.1)
    if np.random.random() < 0.25:
        image1=random_GaussianBlur(image1)
        image_sar1 = random_GaussianBlur(image_sar1)
        
   
    return image,image1,image_sar,image_sar1,label1,label2
def color_aug(image,image_sar):
    band_num=image.shape[2]
    
    if band_num >= 3:
        image[:, :, :3],image_sar[:, :, :3] = randomHueSaturationValue(
            image[:, :, :3],image_sar[:, :, :3], hue_shift_limit=(-30, 30), sat_shift_limit=(-5, 5),
            val_shift_limit=(-15, 15))
        '''
        if band_num > 3:
            for c in range(0, band_num):
...
def random_GaussianBlur(img,img_sar,scale=(1,3)):
    ...
    if channel>3:
        for i in range(channel):
            ...
            img_sar[:, :, i] = cv2.GaussianBlur(img_sar[:, :, i], (k, k), 0)
    else:
        img=cv2.GaussianBlur(img,(k,k),0)
        img_sar = cv2.GaussianBlur(img_sar, (k, k), 0)
    return img,img_sar
...
def random_image_to_gray(image,image_sar, p=0.1):
    #随机灰度化（仅保留其中一个通道）
...
class SegData(data.Dataset):
    ...
    参数：
        选项 - 关于数据集的配置。
            （opt.dataset_dir） - 训练（测试）数据集文件夹的目录：
            ├── root
            |  ├── train
            |  ├── train_labels
            |  ├── val
    ...
    def __init__(
        ...
        #for dt in self.dtype:
            #print('\t', dt)
        self.imgs_sar = {}
        self.imgs_sar = filelist_fromtxt(self.root + '/%s_%s' % (split1, 'SAR'),self.root + '/%s_%s.txt' % (split, 'SAR'), ifPath=True,ifarray=self.quick_train)
        ...
        print('%s set contains %d images_sar, a total of  categories.' %
              (split, len(self.imgs_sar)))  # , len(opt.category.table)))
...
class Train_Dataset(SegData):
    ...
    def __getitem__(self, index):
        ...
        if self.quick_train:
            ...
            image_sar=self.imgs_sar[index].copy()
        else:
            image=io.imread(self.imgs[index])
            image_sar = io.imread(self.imgs_sar[index])

        image,image1,image_sar,image_sar1,lbl1,lbl2=join_transform(image,image_sar,self.lbl.copy())
        ...
        image_sar=image_sar.astype(np.float32).transpose(2, 0, 1).copy() / 255.0 * 3.2 - 1.6
        image_sar1 = image_sar1.astype(np.float32).transpose(2, 0, 1).copy() / 255.0 * 3.2 - 1.6
        ...
        data_dict['image_sar'], data_dict['image_sar1'] = image_sar, image_sar1
...
def get_index(label1,label2,patch_size=(16,16),patch_num=4):
    ...
    # list2 = set(list2)  # this reduces the lookup time from O(n) to O(1)这将查找时间从O（n）减少到O（1）
