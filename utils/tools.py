...
def net_predict1(net, image,image_sar, opt, crop_info=None):
    ''' Do predict use Net(only one image at a time). '''
    if crop_info is None:
        # predict a complete image
        if opt.use_gpu:
                image = image.cuda()
                image_sar = image_sar.cuda()

        output = net(image,image_sar)[0]  # [NCHW] -> [CHW]
        predict = np.argmax(output.cpu().detach().numpy(), 0)  # [CHW] -> [HW]
    else:
        # predict the list of croped images
        predict = []
        image.transpose_(0, 1)  # [NLCHW] -> [LNCHW]
        image_sar.transpose_(0, 1)  # [NLCHW] -> [LNCHW]
        for input,input_sar in image,image_sar:
            if opt.use_gpu:
                input = input.cuda()  # [NCHW](N=1)
                input_sar=input_sar.cuda()
            output = net(input,input_sar)[0]  # [NCHW] -> [CHW]
            # output = output[0]
