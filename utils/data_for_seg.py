...
def random_crop_pair(image,image_sar, label, crop_hw=(256, 256)):
    '''
    ...
    if (crop_h < image.shape[0] and crop_w < image.shape[1]):
        ...
        return image[x:x + crop_h, y:y + crop_w],image_sar[x:x + crop_h, y:y + crop_w], label[
            x:x + crop_h, y:y + crop_w]
    elif (crop_h == image.shape[0] and crop_w == image.shape[1]):
        return image, image_sar,label
    else:
...
def join_transform(image,image_sar, label, input_size):
    """
    ...
    if f != 2:
        image,image_sar, label = filp_array(image, f), filp_array(image_sar, f),filp_array(label, f)
    # Random Roate (Only 0, 90, 180, 270)
    k = np.random.randint(0, 4)  # [0, 1, 2, 3]
    image = np.rot90(image, k, (1, 0))  # clockwise
    image_sar = np.rot90(image_sar, k, (1, 0))
    label = np.rot90(label, k, (1, 0))

    # Random Crop
    # image, label = join_randomScaleAspctCrop(image, label)
    image,image_sar, label = random_crop_pair(image,image_sar, label, input_size)
    ...
    image_sar, label = randomShiftScaleRotate(
        image_sar, label, shift_limit=(-0.1, 0.1), scale_limit=(-0.1, 0.1),
        aspect_limit=(-0.1, 0.1), rotate_limit=(-0, 0))
    if band_num >= 3:
        image_sar[:, :, :3] = randomHueSaturationValue(
            image_sar[:, :, :3], hue_shift_limit=(-30, 30), sat_shift_limit=(-5, 5),
            val_shift_limit=(-15, 15))
        if band_num > 3:
            for c in range(3, band_num):
                image_sar[:, :, c] = randomColorAugment(image_sar[:, :, c], 0.01, 0.01)
                image_sar[:, :, c] = randpixel_noise(image_sar[:, :, c], p=0.001)
    elif band_num < 3:
        for c in range(0, band_num):
            image_sar[:, :, c] = randomColorAugment(image_sar[:, :, c], 0.01, 0.01)
            image_sar[:, :, c] = randpixel_noise(image_sar[:, :, c], p=0.001)
...
class SegData(data.Dataset):
    ...
    def __init__(
       ...
       # self.class_num = opt.class_num_total
        ...
        # Collect all dataset files, divide into dict.收集所有数据集文件，分为字典。
        ...
        self.lbls = filelist_fromtxt(self.root + '/%s_%s' % (split1, 'lbl'),self.root + '/%s_%s.txt' % (split, 'lbl'), ifPath=True,ifarray=self.quick_train)
    ...
    def __getitem__(self, index):
        ...
        if SegData.need_crop(self, image.shape):
            if self.crop_mode == 'random':
                ...
                image,image_sar, lbl = random_crop_pair(image, image_sar,lbl, self.insize)
            ...
            elif self.crop_mode == 'slide':#暂时不考虑
        ...
        data_dict['image'],data_dict['image_sar'], data_dict['label'] = image, image_sar,lbl
        return data_dict
...
class Train_Dataset(SegData):
    ...
    def __getitem__(self, index):
        ...
        lbl = np.expand_dims(lbl, axis=2)
        image,image_sar, lbl = join_transform(image,image_sar, lbl, self.insize)
