...
class DeepLab(nn.Module):
    def __init__(self,
        ...
        self.convfusion_x = nn.Conv2d(4096, 2048, kernel_size=3, padding=1)
        self.convfusion_low = nn.Conv2d(512, 256, kernel_size=3, padding=1)
    ...
    def forward(self, input,input1,input_sar,input_sar1,rois):
        ...
        if not self.noLocal:
            ...
            x, low_level_features = self.backbone_sar(input_sar)
            x = self.encoder(x)
            if not self.noLocal:
                predict = self.decoder(x.clone(), low_level_features)
                predict = F.interpolate(
            ...
            x, low_level_features = self.backbone_sar(input_sar1)
            x = self.encoder(x)
            if not self.noLocal:
                predict1 = self.decoder(x.clone(), low_level_features)
                predict1 = F.interpolate(
